...
def convert_to_text(month):
    lista1 = ['Ano Todo','Janeiro', 'Fevereiro', 'Março', 'Abril', 'Maio', 'Junho', 'Julho', 'Agosto', 'Setembro', 'Outubro', 'Novembro', 'Dezembro']
    return lista1[month]
...
def graph5(team, toggle):
    ...
    fig5.add_trace(go.Indicator(mode='number+delta',
        title = {"text": f"<span>{df_5['Consultor'].iloc[0]} - Top Consultant</span><br><span style='font-size:70%'>Em vendas - em migrations a média</span><br>"},
        value = df_5['Valor Pago'].iloc[0],
        number = {'prefix': "R$"},
        delta = {'relative': True, 'valueformat': '.1%', 'reference': df_5['Valor Pago'].mean()}
    ))

    df_6 = df_6.groupby(['Consultor', 'Equipe'])['Chamadas Realizadas'].sum()
    df_6.sort_values(ascending=False, inplace=True)
    df_6 = df_6.reset_index()
    fig6 =  go.Figure()
    fig6.add_trace(go.Indicator(mode='number+delta',
        title = {"text": f"<span>{df_6['Consultor'].iloc[0]} - Top Consultant</span><br><span style='font-size:70%'>Em chamadas - em migrations a média</span><br>"},
        value = df_6['Chamadas Realizadas'].iloc[0],
        number = {'prefix': "R$"},
        delta = {'relative': True, 'valueformat': '.1%', 'reference': df_6['Chamadas Realizadas'].mean()}
    ))

    fig5.update_layout(main_config, height=200, template=template)
    fig6.update_layout(main_config, height=200, template=template)
    fig5.update_layout({"margin": {"t": 50, "b": 0, "l": 0, "r": 0}})
    fig6.update_layout({"margin": {"t": 50, "b": 0, "l": 0, "r": 0}})
...
def graph7(toggle):
    themplate = template_theme1 if toggle else template_theme2
    ...
    fig7.update_layout(main_config, yaxis={'title': None}, xaxis={'title': None}, height=190, template=themplate)
...
def graph11(month, team, toggle):
    ...
    fig11.update_layout(main_config, height=250, template=template) 
